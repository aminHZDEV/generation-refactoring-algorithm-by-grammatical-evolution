...
from Dependencies.SymbolTable import Program
...
class PullUpMethodRefactoringListener(JavaParserLabeledListener):
    ...
    def exitCompilationUnit(self, ctx: JavaParserLabeled.CompilationUnitContext):
        ...
        self.token_stream_rewriter.insertAfter(
            index=ctx.stop.tokenIndex, text=self.code
        )
...
def get_removed_methods(
    ...
    for d in extendedclass:
        ...
        for mk in class_.methods:
            ...
            m = mk[: mk.find("(")]
            if (
                m_.body_text == body_text_method
                and m_.returntype == returntypeofmethod
                and m_.parameters == parammethod
                and m_.name == nameofmethod
                and m_.is_constructor == False
            ):
...
def main(udb_path: str, children_classes: list, method_name: str, *args, **kwargs):
    ...
    try:
        method_ents = [
            db.lookup(i + "." + method_name, "method")[0] for i in children_classes
        ]
    except IndexError:
