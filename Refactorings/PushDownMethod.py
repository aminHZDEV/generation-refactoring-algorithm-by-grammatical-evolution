...
class PropagationNonStaticListener(PropagationListener):
    ...
    def enterMethodCall0(self, ctx: JavaParserLabeled.MethodCall0Context):
        if (
            ctx.IDENTIFIER().getText() == self.method_name
            and self.is_safe
            and self.detected_class
        ):
            # Change Name
...
def main(
    ...
    **kwargs,
    ...
    my_listener = DeleteSourceListener(
        common_token_stream=token_stream, source_method=source_method
    )
    ...
    # Propagation
    for file, _class, line in zip(
        propagation_files, propagation_classes, propagation_lines
    ):
