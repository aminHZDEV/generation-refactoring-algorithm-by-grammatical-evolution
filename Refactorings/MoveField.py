...
class PropagateListener(JavaParserLabeledListener):
    def __init__(
        self, field_name: str, new_name: str, lines: list, rewriter: TokenStreamRewriter
    ):
        self.field_name = field_name
    ...
    def enterExpression1(self, ctx: JavaParserLabeled.Expression1Context):
        ...
        if (
            ...
            and identifier.getText() == self.field_name
        ):
            self.rewriter.replaceSingleToken(token=ctx.stop, text=self.new_name)
    ...
    def enterExpression0(self, ctx: JavaParserLabeled.Expression0Context):
        ...
        if (
            identifier
            and ctx.start.line in self.lines
            and identifier == self.field_name
        ):
            self.rewriter.replaceSingleToken(token=ctx.stop, text=self.new_name)
