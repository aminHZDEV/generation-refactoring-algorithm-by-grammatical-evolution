...
from Dependencies.SymbolTable import parse_and_walk, Program
    ...
    for d in extendedclass:
        ...
        for mk in class_.methods:
            ...
            if (
                ...
                listBody_text = (
                    body_text_method.replace("{", "").replace("}", "").split(";")
                )
...
def main(udb_path, source_package, target_class, class_names: list, *args, **kwargs):
    ...
    if len(parent) != 1:
        print("Count of target class is not 1.")
        db.close()
        return False
    parent = parent[0]
