...
from Refactorings.MoveField import CheckCycleListener
from Dependencies.SymbolTable import parse_and_walk
    ...
    def enterPrimary4(self, ctx: JavaParserLabeled.Primary4Context):
        if self.fields:
            ...
            if field_name in self.fields:
                ...
                self.rewriter.insertBeforeToken(token=ctx.start, text="ref.")
...
class PropagateListener(JavaParserLabeledListener):
    ...
    def enterMethodCall0(self, ctx: JavaParserLabeled.MethodCall0Context):
        ...
        if (
            identifier
            and ctx.start.line in self.lines
            and identifier.getText() == self.method_name
        ):
            if self.fields:
            ...
            if self.is_in_target_class:
                self.rewriter.replaceSingleToken(token=ctx.parentCtx.start, text="this")
            else:
                self.rewriter.replaceSingleToken(token=ctx.start, text=self.new_name)
...
def main(
    ...
    **kwargs,
    ...
    try:
        src_class_file = db.lookup(f"{source_package}.{source_class}.java", "File")[
            0
        ].longname()
        target_class_file = db.lookup(f"{target_package}.{target_class}.java", "File")[
            0
        ].longname()
    except IndexError:
