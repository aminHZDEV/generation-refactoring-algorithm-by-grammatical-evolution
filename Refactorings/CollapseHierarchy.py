...
from Refactorings.RemoveClass import RemoveClassRefactoringListener
...
class CollapseHierarchyRefactoringGetFieldTextListener(JavaParserLabeledListener):
    ...
    def enterClassDeclaration(self, ctx: JavaParserLabeled.ClassDeclarationContext):
        ...
        if class_identifier == self.child_class:
            ...
            self.fieldcode += (
                f"// child class({self.child_class}) fields: " + self.NEW_LINE
            )
        else:
    ...
    def exitCompilationUnit(self, ctx: JavaParserLabeled.CompilationUnitContext):
        self.token_stream_rewriter.insertAfter(
            index=ctx.stop.tokenIndex, text=self.fieldcode
        )
...
class CollapseHierarchyRefactoringGetMethodTextListener(JavaParserLabeledListener):
    ...
    def enterClassDeclaration(self, ctx: JavaParserLabeled.ClassDeclarationContext):
        ...
        if class_identifier == self.child_class:
            ...
            self.methodcode += (
                f"// child class({self.child_class}) methods: " + self.NEW_LINE
            )
        else:
            self.is_source_class = False
    ...
    def exitCompilationUnit(self, ctx: JavaParserLabeled.CompilationUnitContext):
        self.token_stream_rewriter.insertAfter(
            index=ctx.stop.tokenIndex, text=self.methodcode
        )
    ...
    def exitMethodDeclaration(self, ctx: JavaParserLabeled.MethodDeclarationContext):
        ...
        if self.detected_method == method_identifier:
            ...
            self.methodcode += (
                self.NEW_LINE + self.TAB + modifier + method_text + self.NEW_LINE
            )
...
class PropagationCollapseHierarchyListener(JavaParserLabeledListener):
    def __init__(
        self,
        token_stream_rewriter: CommonTokenStream = None,
        old_class_name: list = None,
        new_class_name: str = None,
        propagated_class_name: list = None,
    ):
    ...
    def enterVariableDeclarator(self, ctx: JavaParserLabeled.VariableDeclaratorContext):
        ...
        if class_identifier in self.old_class_name:
            ...
            grand_child_ctx = (
                ctx.variableInitializer().expression().creator().createdName()
            )
            self.token_stream_rewriter.replaceRange(
