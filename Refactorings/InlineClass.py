...
from Refactorings.CollapseHierarchy import CollapseHierarchyRefactoringListener
from gen.javaLabeled.JavaLexer import JavaLexer
from gen.javaLabeled.JavaParserLabeled import JavaParserLabeled
from Refactorings.MakeMethodStatic import MakeMethodStaticRefactoringListener
from Refactorings.MakeMethodNonStatic import MakeMethodNonStaticRefactoringListener
import os
...
class InlineClassRefactoringListener(JavaParserLabeledListener):
    ...
    def __init__(
        ...
        source_class_data: dict = None,
        target_class: str = None,
        target_class_data: dict = None,
    ...
    def exitClassDeclaration(self, ctx: JavaParserLabeled.ClassDeclarationContext):
        if self.is_target_class and (
            self.source_class_data["fields"]
            or self.source_class_data["constructors"]
            or self.source_class_data["methods"]
        ):
            if not self.is_complete:
                final_fields = merge_fields(
                    self.source_class_data["fields"],
                    self.target_class_data["fields"],
                    self.target_class,
                )
                final_constructors = merge_constructors(
                    self.source_class_data["constructors"],
                    self.target_class_data["constructors"],
                )
                final_methods = merge_methods(
                    self.source_class_data["methods"], self.target_class_data["methods"]
                )
                text = "\t"
                ...
                self.token_stream_rewriter.insertBeforeIndex(
                    index=ctx.stop.tokenIndex, text=text
                )
    ...
    def enterFieldDeclaration(self, ctx: JavaParserLabeled.FieldDeclarationContext):
        if self.is_source_class or self.is_target_class:
            ...
            if (
                ctx.typeType().classOrInterfaceType() is not None
                and ctx.typeType().classOrInterfaceType().getText() == self.source_class
            ):
        ...
        if self.is_source_class or self.is_target_class:
            if ctx.formalParameters().formalParameterList():
                constructor_parameters = [
                    ...
                    for i in range(
                        len(ctx.formalParameters().formalParameterList().children)
                    )
                    if i % 2 == 0
                ]
            else:
            ...
            constructor_text += "}\n"
            if self.is_source_class:
                self.source_class_data["constructors"].append(
                    ConstructorOrMethod(
                        ...
                        constructor_body=self.token_stream_rewriter.getText(
                            program_name=self.token_stream_rewriter.DEFAULT_PROGRAM_NAME,
                            start=ctx.block().start.tokenIndex + 1,
                            stop=ctx.block().stop.tokenIndex - 1,
                        ),
                ...
                proper_constructor = get_proper_constructor(
                    self.target_class_data["constructors"][-1],
                    self.source_class_data["constructors"],
                )
                ...
                self.token_stream_rewriter.insertBeforeIndex(
                    index=ctx.stop.tokenIndex, text=proper_constructor.constructorBody
                )
    ...
    def enterMethodDeclaration(self, ctx: JavaParserLabeled.MethodDeclarationContext):
        if self.is_source_class or self.is_target_class:
            ...
            method_text += "}\n"
            if self.is_source_class:
                self.source_class_data["methods"].append(
                    ConstructorOrMethod(
                ...
                self.target_class_data["methods"].append(
                    ConstructorOrMethod(
                        name=ctx.IDENTIFIER().getText(),
                        parameters=[
                            Parameter(
    ...
    def enterExpression1(self, ctx: JavaParserLabeled.Expression1Context):
        if (
            ...
            and ctx.IDENTIFIER().getText() in self.field_that_has_source
        ):
    ...
    def enterCreatedName0(self, ctx: JavaParserLabeled.CreatedName0Context):
        if ctx.IDENTIFIER(0).getText() == self.source_class and self.target_class:
            self.token_stream_rewriter.replaceIndex(
                index=ctx.start.tokenIndex, text=self.target_class
            )
    ...
    def enterFormalParameter(self, ctx: JavaParserLabeled.FormalParameterContext):
        ...
        if class_type:
            if (
                class_type.IDENTIFIER(0).getText() == self.source_class
                and self.target_class
            ):
                self.token_stream_rewriter.replaceIndex(
                    index=class_type.start.tokenIndex, text=self.target_class
                )
...
def is_equal_constructor(
    first_constructor: ConstructorOrMethod, second_constructor: ConstructorOrMethod
):
    if len(first_constructor.parameters) == len(second_constructor.parameters):
        second_constructor_params = [
            param.parameterType for param in second_constructor.parameters
        ]
...
def merge_constructors(
    source_constructors: ConstructorOrMethod, target_constructors: ConstructorOrMethod
):
    final_constructors = []
    for source_constructor in source_constructors:
        ...
        for target_constructor in target_constructors:
            if source_constructor.name == target_constructor.name:
                if is_equal_constructor(
                    source_constructor, target_constructor
                ):  # check equality of two constructor
                    flag = False
        if flag:
...
def merge_methods(
    source_methods: ConstructorOrMethod, target_methods: ConstructorOrMethod
):
    final_methods = []
    for source_method in source_methods:
        ...
        for target_method in target_methods:
            if source_method.name == target_method.name:
                if len(source_method.parameters) == len(
                    target_method.parameters
                ):  # change equality of two methods
                    flag = False
        if flag:
...
def get_proper_constructor(
    target_constructor: ConstructorOrMethod, source_constructors: ConstructorOrMethod
):
    for source_constructor in source_constructors:
...
def main(project_path: str = "", refactoring_id: str = "i"):
    input_directory = project_path
    project_name = input_directory.split("/")[-1]
    ...
    for i in range(2):
        for file in input_java_files:
            ...
            if i == 0:
                stream = FileStream(
                    input_directory + "/" + file, encoding="utf8", errors="ignore"
                )
            else:
                stream = FileStream(
                    "Resources/projects_refactored/" + project_name + "/" + file,
                    encoding="utf8",
                    errors="ignore",
                )
            ...
            if refactoring_id == "c":
                my_listener = CollapseHierarchyRefactoringListener(
                    ...
                    source_class="JSONStringer",
                    target_class=target_class,
                    source_class_data=source_class_data,
                ...
                my_listener = InlineClassRefactoringListener(
                    common_token_stream=token_stream,
                    source_class="HTTPTokener",
                    target_class="JSONTokener",
                    source_class_data=source_class_data,
                    target_class_data=target_class_data,
                    is_complete=is_complete,
                )
