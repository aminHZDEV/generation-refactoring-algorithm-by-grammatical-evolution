...
class DependencyPreConditionListener(JavaParserLabeledListener):
    ...
    def split_class(self):
        ...
        for class_ in S:
            class_fields = [
                node for node in class_.nodes if class_.in_degree(node) == 0
            ]
            class_methods = [
                class_.nodes[node]["method_name"]
                for node in class_.nodes
                if class_.in_degree(node) > 0
            ]
...
class ExtractClassRefactoringListener(JavaParserLabeledListener):
    ...
    def __init__(
        ...
        for method in moved_methods:
            if (
                self.method_map.get(method) is not None
                and len(self.method_map.get(method)) > 0
            ):
    ...
    def enterClassDeclaration(self, ctx: JavaParserLabeled.ClassDeclarationContext):
        ...
        if class_identifier == self.source_class:
            ...
            self.code += (
                f"// New class({self.new_class}) generated by CodART" + self.NEW_LINE
            )
    ...
    def enterFormalParameter(self, ctx: JavaParserLabeled.FormalParameterContext):
        if self.detected_method:
            self.parameters.append(ctx.variableDeclaratorId().IDENTIFIER().getText())
    ...
    def enterExpression1(self, ctx: JavaParserLabeled.Expression1Context):
        ...
        if identifier is not None:
            if (
                identifier.getText() in self.moved_fields
                and self.detected_method not in self.moved_methods
            ):
                # Found field usage!
...
class PropagateFieldUsageListener(JavaParserLabeledListener):
    def __init__(
        self, common_token_stream: CommonTokenStream, object_name: str, field_name: str
    ):
        self.token_stream_rewriter = TokenStreamRewriter(common_token_stream)
...
class ExtractClassAPI:
    ...
    def check_dependency_graph(self):
        listener = DependencyPreConditionListener(
            common_token_stream=self.token_stream, class_identifier=self.source_class
        )
    ...
    def get_source_class_map(self):
        ...
        for ref in class_ent.refs("Define", "Method"):
            ...
            for use in method_ent.refs(
                "Setby Useby Modifyby, Call", "Variable ~Unknown, Method ~Unknown"
            ):
    ...
    def propagate_fields(self, usages):
        for usage in usages:
            ...
            my_listener = PropagateFieldUsageListener(
                common_token_stream=token_stream, object_name=self.object_name, **usage
            )
    ...
    def do_refactor(self):
        ...
        # Write Changes
        with open(
            self.file_path, mode="w", encoding="utf-8", errors="ignore", newline=""
        ) as f:
            f.write(listener.token_stream_rewriter.getDefaultText())

        # Write new class
        with open(
            self.new_file_path, mode="w", encoding="utf-8", errors="ignore", newline=""
        ) as f:
            f.write(my_listener.token_stream_rewriter.getDefaultText())
...
def main(
    udb_path, file_path, source_class, moved_fields, moved_methods, *args, **kwargs
):
    new_class = f"{source_class}Extracted"
    new_file_path = os.path.join(Path(file_path).parent, f"{new_class}.java")

    if not os.path.exists(file_path):
        print(f'The source class "{source_class}" is nested in {file_path}')
        return False

    if os.path.exists(new_file_path):
        print(f'The new class "{new_file_path}" already exist.')
        return False
    ...
    if len(eca.method_usage_map) == 0:
        print(f"The method_usage_map is empty: {len(eca.method_usage_map)}")
        return False
    else:
        res = eca.do_refactor()
        return res
