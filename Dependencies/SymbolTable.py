...
class TokensInfo:
    ...
    def __init__(self, parser_context=None):
        if parser_context is not None:
            self.token_stream: CommonTokenStream = (
                parser_context.parser.getTokenStream()
            )
...
class FileInfo:
    ...
    def has_imported_class(self, package_name: str, class_name: str) -> bool:
        if self.package_name == package_name:
            return True
        return any(
            lambda x: x.package_name == package_name
            for package_import in self.package_imports
        ) or any(
            lambda x: x.package_name == package_name and x.class_name == class_name
            for class_import in self.class_imports
        )
...
class SingleFileElement:
    ...
    def __init__(
        self, parser_context, filename: str = None, _file_info: FileInfo = None
    ):
...
class Class(SingleFileElement):
    ...
    def __str__(self):
        return (
            ...
            + str(self.name)
            + (
                (" extends " + str(self.superclass_name))
                if self.superclass_name is not None
                else ""
            )
            + (
                (" implements " + str(self.superinterface_names))
                if len(self.superinterface_names) > 0
                else ""
            )
            + " "
            + str(self.fields)
...
class Method(SingleFileElement):
    def __init__(
        ...
        self.body_local_vars_and_expr_names = (
            []
        )  # Type: either LocalVariable, ExpressionName or MethodInvocation
        self.package_name = package_name
        self.class_name = class_name
        self.file_info = file_info
...
class ExpressionName:
    def __init__(
        self, dot_separated_identifiers: list, ctx: JavaParser.ExpressionContext
    ):
...
class MethodInvocation:
    def __init__(
        self,
        dot_separated_identifiers: list,
        parser_context: JavaParser.ExpressionContext = None,
    ):
        self.dot_separated_identifiers = dot_separated_identifiers
        self.parser_context = parser_context
...
class UtilsListener(JavaParserListener):
    ...
    def enterTypeDeclaration(self, ctx: JavaParser.TypeDeclarationContext):
        ...
        for modifier in ctx.getChildren(
            lambda x: type(x) == JavaParser.ClassOrInterfaceModifierContext
        ):
    ...
    def enterClassBodyDeclaration(self, ctx: JavaParser.ClassBodyDeclarationContext):
        ...
        self.last_modifiers_contexts.clear()
        for modifier in ctx.getChildren(
            lambda x: type(x) == JavaParser.ModifierContext
        ):
            self.last_modifiers.append(modifier.getText())
            self.last_modifiers_contexts.append(modifier)
    ...
    def enterClassDeclaration(self, ctx: JavaParser.ClassDeclarationContext):
        if self.current_class_identifier is None and self.nest_count == 0:
            ...
            current_class.modifiers_parser_contexts = (
                self.last_modifiers_contexts.copy()
            )
            ...
            if ctx.IMPLEMENTS() is not None:
                for interface_type in ctx.typeList().getChildren(
                    lambda x: type(x) == JavaParser.TypeTypeContext
                ):
    ...
    def enterMethodDeclaration(self, ctx: JavaParser.MethodDeclarationContext):
        if self.current_class_identifier is not None:
            ...
            self.objects_declaration[self.current_class_identifier][
                self.current_method_identifier
            ] = {}
    ...
    def general_exit_method_decl(self):
        if self.current_class_identifier is not None:
            if self.current_method is not None:
                ...
                method_key += ")"
                self.package.classes[self.current_class_identifier].methods[
                    method_key
                ] = method
        self.current_method_identifier = None
        self.current_method = None
        ...
        if self.current_method is not None:
            if len(ctx.parentCtx.children) == 3:
                ...
                if object_or_class not in self.current_method.body_method_invocations:
                    self.current_method.body_method_invocations[object_or_class] = [
                        ctx.IDENTIFIER().getText()
                    ]
                else:
                    self.current_method.body_method_invocations[object_or_class].append(
                        ctx.IDENTIFIER().getText()
                    )

            elif len(ctx.parentCtx.children) == 1 and ctx.IDENTIFIER():
                if (
                    self.current_class_ctx
                    not in self.current_method.body_method_invocations_without_typename
                ):
    ...
    def enterCreator(self, ctx: JavaParser.CreatorContext):
        try:
            object_name = (
                ctx.parentCtx.parentCtx.parentCtx.children[0].IDENTIFIER().getText()
            )
            class_name = (
                ctx.parentCtx.parentCtx.parentCtx.parentCtx.parentCtx.children[0]
                .children[0]
                .getText()
            )
            self.objects_declaration[self.current_class_identifier][
    ...
    def exitFieldDeclaration(self, ctx: JavaParser.FieldDeclarationContext):
        ...
        if self.current_class_identifier is not None and self.field_enter_count == 0:
            for i in range(len(self.current_field_ids)):
                ...
                field.neighbor_names = [
                    x for x in self.current_field_ids if x != field_id
                ]
                field.all_variable_declarator_contexts = self.current_field_var_ctxs
                field.index_in_variable_declarators = i
                self.package.classes[self.current_class_identifier].fields[
                    field.name
                ] = field
            self.current_field_decl = None
...
class FieldUsageListener(UtilsListener):
    """
    FieldUsageListener finds all the usage of an specified field f, from a class c in package pkg.
    """
    ...
    def enterClassDeclaration(self, ctx: JavaParser.ClassDeclarationContext):
        ...
        self.has_imported_source = self.file_info.has_imported_package(
            self.package.name
    ...
    def enterClassBody(self, ctx: JavaParser.ClassBodyContext):
        ...
        if self.current_class_name == self.target_class:
            ...
            if self.field_tobe_moved.name == self.field_name:
                ...
                replacement_text += (
                    f"{self.field_tobe_moved.datatype} {self.field_tobe_moved.name};"
                )
            ...
            method_name = (
                self.field_tobe_moved.name.upper() + self.field_tobe_moved.name[1:-1]
            )
    ...
    def exitMethodDeclaration(self, ctx: JavaParser.MethodDeclarationContext):
        ...
        if (
            self.current_class_name == self.source_class
            and self.is_method_getter_or_setter(ctx.IDENTIFIER().getText())
        ):
    ...
    def handleMethodUsage(self, ctx, is_constructor: bool):
        method_identifier = (
            ctx.IDENTIFIER().getText()
            if is_constructor
            else ctx.parentCtx.IDENTIFIER().getText()
        )
        formal_params = (
            ctx.formalParameters()
            if is_constructor
            else ctx.parentCtx.formalParameters()
        )
        target_added = False
        target_param_name = "$$target"
        target_param = (
            f"Target {target_param_name}"
            if len(self.current_method.parameters) == 0
        ...
        for var_or_exprs in self.current_method.body_local_vars_and_expr_names:
            if type(var_or_exprs) is ExpressionName:
                ...
                try:
                    local_ctx = (
                        var_or_exprs.parser_context.parentCtx.parentCtx.parentCtx.parentCtx.parentCtx.parentCtx
                    )
                    creator = local_ctx.expression()[0].getText()
                    if (
                        creator.__contains__(f"new{self.source_class}")
                ...
                if len(var_or_exprs.dot_separated_identifiers) < 2:
                    continue
                if (
                    var_or_exprs.dot_separated_identifiers[0] in local_candidates
                    or var_or_exprs.dot_separated_identifiers[0]
                    in self.field_candidates
                ) and var_or_exprs.dot_separated_identifiers[1] == self.field_name:
                    if not target_added:
                        # add target to param
                        self.rewriter.insertBeforeIndex(
                ...
                if (
                    ...
                    if (
                        var_or_exprs.parser_context.methodCall() is not None
                        and self.is_method_getter_or_setter(
                            var_or_exprs.parser_context.methodCall()
                            .IDENTIFIER()
                            .getText()
                        )
                    ):
                    ...
                    if (
                        not should_ignore
                        and var_or_exprs.parser_context is not None
                        and type(var_or_exprs.parser_context)
                    ...
                    var_or_exprs.dot_separated_identifiers
    ...
    def is_getter_or_setter(self, first_id: str, second_id: str, local_candidates: set):
        return (first_id in local_candidates or first_id in self.field_candidates) and (
            second_id == f"set{self.field_name[0].upper() + self.field_name[1:-1]}"
            or second_id == f"get{self.field_name[0].upper() + self.field_name[1:-1]}"
            or second_id == f"has{self.field_name[0].upper() + self.field_name[1:-1]}"
            or second_id == f"is{self.field_name[0].upper() + self.field_name[1:-1]}"
        )

    def is_method_getter_or_setter(self, method: str):
        return (
            method == f"set{self.field_name[0].upper() + self.field_name[1:-1]}"
            or method == f"get{self.field_name[0].upper() + self.field_name[1:-1]}"
            or method == f"has{self.field_name[0].upper() + self.field_name[1:-1]}"
            or method == f"is{self.field_name[0].upper() + self.field_name[1:-1]}"
        )

    def propagate_getter_setter(
        self, ctx: JavaParser.ExpressionContext, target_name: str
    ):
        index = ctx.DOT().symbol.tokenIndex
        self.rewriter.replaceRange(ctx.start.tokenIndex, index - 1, target_name)

    def propagate_getter_setter_form2(
        self, ctx: JavaParser.ExpressionContext, target_name: str
    ):
        """
        form 2 is getA() setA()...
...
class MethodUsageListener(UtilsListener):
    ...
    def enterClassCreatorRest(self, ctx: JavaParser.ClassCreatorRestContext):
        if type(ctx.parentCtx) is JavaParser.CreatorContext:
            if (
                ctx.parentCtx.createdName().IDENTIFIER()[0].getText()
                not in self.method_names
            ):
                return
        text = (
            f"new {self.target_class}()"
            if ctx.arguments().expressionList() is None
            else f", new {self.target_class}()"
        )
...
def get_filenames_in_dir(
    directory_name: str, filter_=lambda x: x.endswith(".java")
) -> list:
    result = []
    ...
    for filename in source_files:
        ...
        if not (listener_package_name in program.packages):
            ...
            for classes_name in listener.package.classes:
                program.packages[listener_package_name].classes[
                    classes_name
                ] = listener.package.classes[classes_name]
    return program
        ...
        if not (listener.package.name in objects):
            ...
            for class_name in listener.objects_declaration:
                objects[listener.package.name][
                    class_name
                ] = listener.objects_declaration[class_name]
...
class Rewriter:
    def __init__(
        self, program: Program, filename_mapping=lambda x: x + ".rewritten.java"
    ):
    ...
    def get_token_stream_rewriter(
        self, token_stream: CommonTokenStream
    ) -> TokenStreamRewriter:
        return self.token_streams[token_stream][1]

    def replace(self, tokens_info: TokensInfo, text: str):
        self.get_token_stream_rewriter(tokens_info.token_stream).replaceRange(
            tokens_info.start, tokens_info.stop, text
        )

    def insert_after(self, tokens_info: TokensInfo, text: str):
        self.get_token_stream_rewriter(tokens_info.token_stream).insertAfter(
            tokens_info.stop, text
        )

    def insert_before(self, tokens_info: TokensInfo, text: str):
        self.get_token_stream_rewriter(tokens_info.token_stream).insertBeforeIndex(
            tokens_info.stop, text
        )

    def insert_before_start(self, tokens_info: TokensInfo, text: str):
        self.get_token_stream_rewriter(tokens_info.token_stream).insertBeforeIndex(
            tokens_info.start, text
        )
    ...
    def apply(self):
        for token_stream in self.token_streams:
            (old_filename, token_stream_rewriter, new_filename) = self.token_streams[
                token_stream
            ]
...
def get_program_with_field_usage(
    source_files: list, field_name: str, source_class: str, print_status=False
) -> Program:
    program = Program()
    for filename in source_files:
        if print_status:
...
def create_project_parse_tree(java_file_path):
    ...
    try:
        ...
        sa_javalabeled.USE_CPP_IMPLEMENTATION = True
...
def parse_and_walk(
    file_path: str, listener_class, has_write=False, debug=False, **kwargs
):
    ...
    if has_write:
        if not debug:
            with open(
                file_path, mode="w", encoding="utf-8", errors="ignore", newline=""
            ) as f:
